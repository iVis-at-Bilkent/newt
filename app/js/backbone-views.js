...
var MapTabGeneralPanel = GeneralPropertiesParentView.extend({
  initialize: function() {
    ...
    self.params.inferParentNodeOnLoad = {id: "infer-parent-node-on-load", type: "checkbox",
      property: "currentGeneralProperties.inferParentNodeOnLoad"};
    ...
    $(document).on("change", "#infer-parent-node-on-load", function (evt) {

      // use active cy instance
      var cy = appUtilities.getActiveCy();

      self.params.inferParentNodeOnLoad.value = $('#infer-parent-node-on-load').prop('checked');
      cy.undoRedo().do("changeMenu", self.params.inferParentNodeOnLoad);
      $('#infer-parent-node-on-load').blur();
    });
    ...
    $(document).on("click", "#map-general-default-button", function (evt) {
      ...
      self.params.inferParentNodeOnLoad.value = appUtilities.defaultGeneralProperties.inferParentNodeOnLoad;
      ...
      actions.push({name: "changeMenu", param: self.params.inferParentNodeOnLoad});
